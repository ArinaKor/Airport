...
public interface PlaneRepository extends JpaRepository<Plane, Integer> {
   ...
   /* @Query("from Plane pl WHERE pl.town = :town")
    ...
    @Query("from Plane pl ORDER BY pl.price_ticket")
    List<Plane> findAll1();

    @Query("from Plane pl WHERE pl.airport1 = :airport1 ORDER BY pl.price_ticket")
    List<Plane> findByAirport1(String airport1);

    @Query("from Plane pl WHERE pl.airport2 = :airport2 ORDER BY pl.price_ticket")
    List<Plane> findByAirport2(String airport2);

    @Query("from Plane pl Where pl.date_flight>=:date1 and pl.date_flight<=:date2 ORDER BY pl.price_ticket")
    List<Plane> findByDate1(Date date1, Date date2);

    @Query("from Plane pl Where pl.time_flight>=:time1 and pl.time_flight<=:time2 ORDER BY pl.price_ticket")
